...
const VerifyUser = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const verificationResponse = await axiosSecure.post('/verify-user', verificationData);
        ...
        if (verificationResponse.data.autoVerified) {
          ...
          try {
            ...
            if (createResponse.data.success) {
              alert(`Your ${userType} account has been verified and created successfully!`);
          ...
          alert("Your verification request is pending admin approval. You'll be notified once verified.");

        }
      } else {
        // Handle specific error cases from backend
        if (verificationResponse.data.status === 'verified') {
          alert("This student ID is already registered. Please contact support if this is an error.");
        } else if (verificationResponse.data.status === 'pending') {
          alert("Your verification request is already pending review. Please wait for approval.");
        } else {
          alert(verificationResponse.data.message || "Submission failed. Please try again.");
        }
      }
    } catch (error) {
    ...
    <div className=" max-w-screen-xl mx-auto px-5 lg:px-0 py-16">
      ...
      {!verificationMethod && (
        <div className="grid md:grid-cols-2 gap-6">
          <div
            className={`card cursor-pointer transition-all duration-300 hover:shadow-xl ${verificationMethod === "student" ? "ring-2 ring-blue-500 shadow-lg" : ""
              }`}
          ...
          >
            <div className="card-body">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="card-title text-white">Student Email Verification</h3>
                  <p className="text-gray-400">Verify with your BRACU Email (@g.bracu.ac.bd)</p>
                </div>
              </div>
              <p className="text-sm text-gray-400 mt-4">
                Quick verification for current students using their official BRACU email address.
              </p>
            </div>
          </div>

          <div
            className={`card cursor-pointer transition-all duration-300 hover:shadow-xl ${verificationMethod === "alumni" ? "ring-2 ring-blue-500 shadow-lg" : ""
              }`}
            ...
            <div className="card-body">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-400">Upload documents or provide graduation details</p>
                </div>
              </div>
              <p className="text-sm text-gray-400 mt-4">
                For alumni without active BRACU email. Requires admin approval (24-48 hours).
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="card bg-[#1E293B] border border-[#334155]">
          <div className="card-body">
            ...
            <p className="text-gray-400 mb-6">
              Please provide accurate information to verify your BRACU affiliation.
            </p>
              ...
              <div className="alert alert-info mb-6 bg-blue-500/10 border-blue-500/20">
                <div className="flex items-center">
                  <span className="text-blue-400">
                    You are registering as a <strong className="text-white capitalize">{userType}</strong>
                  </span>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="form-control">
                  ...
                  <input
                    type="text"
                    placeholder={userType === "student" ? "e.g., 20341001" : "e.g., 20101234"}
                    className="input input-bordered bg-[#1E293B] border-[#334155] text-white"
              ...
              <div className="form-control">
                <label className="label">
                  <span className="label-text text-white">Official Email *</span>
                </label>
                <input
                  type="email"
                  placeholder={
              ...
              {userType === "alumni" && (
                <>
                  ...
                  <div className="form-control">
                    <label className="label">
                      <span className="label-text text-white">Current Company (Optional)</span>
                    </label>
                    <input
                      type="text"
                ...
                <div className="alert alert-info bg-blue-500/10 border-blue-500/20">
                  <div className="flex items-center">
                    <span className="text-blue-400 text-sm">
                      Student verification is instant using your BRACU student ID.
                    </span>
                  </div>
                </div>
              )}
                ...
                <div className="alert alert-warning bg-yellow-500/10 border-yellow-500/20">
                  <div className="flex items-center">
                    <span className="text-yellow-400 text-sm">
                      Alumni verification may take 24-48 hours for manual review.
                    </span>
                  </div>
                </div>
              )}
              ...
              <div className="form-control">
                <label className="label cursor-pointer justify-start gap-3">
                  ...
                  <span className="label-text text-white flex-1 ">
                    I confirm that the information provided <br className="md:hidden" /> is correct and belongs to me.
                  </span>
                </label>
              </div>
